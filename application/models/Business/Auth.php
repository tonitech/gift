...
class Business_Auth
{
    ...
    public function checkLogin($username, $password)
    {
        ...
        $rtn = Utility_Db::getInstance()
            ->conn()
            ->select()
            ->from($adminTable->tablename, '*')
            ->where($adminTable->username . '=?', $username)
            ->where($adminTable->password . '=?', md5($password))
            ->query()
            ->fetch();
        if ($rtn) {
            return array(
    ...
    {
        $login = new Zend_Session_Namespace('admin');
        $login->userinfo = $user;
        $login->admin = $this->generateAuthSignature(
            $user,
    ...
    {
        $user = new Zend_Session_Namespace('admin');
		if ($user->admin != '') {
			$user->__unset('admin');
